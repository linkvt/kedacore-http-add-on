...
# -p 1 is needed to run only one profile (=addon configuration) in parallel
	go test -tags e2e $(E2E_PACKAGE) -p 1 -count=1 -timeout 15m -v $(if $(RUN),-run '$(RUN)') $(if $(E2E_ARGS),-args $(E2E_ARGS))
...
# -p 1 is needed to run only one profile (=addon configuration) in parallel
	gotestsum --rerun-fails=2 --format=github-actions --packages="./test/e2e/..." -- -tags e2e -p 1 -count=1 -timeout 30m -v
